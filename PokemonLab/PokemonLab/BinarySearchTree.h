...
public:
	...
	int getNumNodes(Node <T> * toFindSubNodesOf) const{
	...
	bool outInOrder(Node<T> * root, stringstream & out) const {
		if (root == nullptr) {
			return false;
		}

		outInOrder(root->left, out);
		out << root->Data << ",";
		outInOrder(root->right, out);
		return true;
	...
	{
		stringstream out;
		if (root == nullptr) out << "empty ";
	
		outInOrder(root, out);

		string oneExtraSpace = out.str();

		string toReturn = oneExtraSpace.substr(0, oneExtraSpace.size() - 1);
